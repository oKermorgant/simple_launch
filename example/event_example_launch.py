...
def generate_launch_description():
    ...
    with sl.group(when = After(node, OnProcessStart, 1.)):
        other_node = waiting_node('other_node', 3,
                    '1 s after main node starts')

    with sl.group(ns = 'last_ns', when = After(other_node, OnProcessExit, sl.arg('delay'))):
        last = waiting_node('last', 2,
                    'after other_node exits and should terminate with main if delay = 4')
